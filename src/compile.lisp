  ...
  (cond
   ...
   ((case (first x)
      ...
      (env-ref (let ((env-var (second x))
                     (idx (fourth x)))
                 `(,(comp-env-ref env-var idx env) ,env ,tenv)))
...
(defun comp-cast (rettype union variable clauses env tenv)
  (let* ((union-exp (first union))
         ...
         (rettype-ptr (llvm:build-malloc *builder* (llvm-alloc rettype tenv) ""))
         ...
         (retptr (llvm:build-alloca *builder* (llvm-type rettype tenv) ""))
    ...
    (map nil
         #'(lambda (tag type body basic-block)
             ...
             (let* ((type* (llvm-type type tenv))
                    (alloca (llvm:build-alloca *builder* type* ""))
                    (data-bit-cast (llvm:build-bit-cast *builder* data-ptr type* "")))
               (llvm:build-store *builder* data-bit-cast alloca)
               (let* ((env* `((,variable ,alloca) . ,env))
    ...
    `(,(llvm:build-load *builder* retptr "") ,env ,tenv)))
...
(defun comp-quote (const type env tenv)
  (cond
    ((eq type 'i32) `(,(llvm:const-int (llvm:int32-type) const) ,env ,tenv))
    ((symbolp type) `(,(llvm:build-global-string *builder* (string const) "")
                       ,env ,tenv))
    ((case (first type)
       (structure
        (let* ((elements (rest const))
               (types (rest type))
               (types* (map 'list #'(lambda (x) (llvm-type x tenv)) types))
          ...
          (map nil
               #'(lambda (const type)
                   (let* ((cconst (first (comp-quote const type env tenv)))
                     ...
                     (llvm:build-store *builder* cconst element-ptr)
                     (setf idx (1+ idx))))
               elements types)
          `(,ptr ,env ,tenv)))))))
...
(defun comp-cons (elements types env tenv)
  (let* ((types* (map 'list #'(lambda (x) (llvm-type x tenv)) types))
         (struct-type (llvm:struct-type (coerce types* 'vector) nil))
         (ptr (llvm:build-malloc *builder* struct-type ""))
         (idx 0))
    (map nil
         #'(lambda (exp)
             (let* ((cexp (first (comp exp env tenv)))
                    (indices (vector (llvm:const-int (llvm:int32-type) 0)
                                     (llvm:const-int (llvm:int32-type) idx)))
                    (element-ptr (llvm:build-gep *builder* ptr indices "")))
               (llvm:build-store *builder* cexp element-ptr)
               (setf idx (1+ idx))))
         elements)
    `(,ptr ,env ,tenv)))
...
(defun comp-make-closure (c-make-env clambda*)
  (if (and c-make-env clambda*)
      (let* ((element-types (vector (llvm:type-of clambda*) (llvm:type-of c-make-env)))
             ...
             (ptr (llvm:build-malloc *builder* closure-type ""))
             (idx 0))
        (if ptr
            (progn
              (map nil
              ...
              ptr)
            (error 'satori-error :message "unable to allocate memory")))
    ...
    (map nil
         #'(lambda (x)
             (let* ((indices (vector (llvm:const-int (llvm:int32-type) 0)
                                     (llvm:const-int (llvm:int32-type) idx)))
                    (var-ptr (llvm:build-gep *builder* ptr indices ""))
                    (cvar (first (comp x env tenv*))))
               (llvm:build-store *builder* cvar var-ptr)
               (setf idx (1+ idx))))
         vs)
    `(,ptr ,tenv* ,env*)))

(defun comp-env-ref (env-var idx env)
