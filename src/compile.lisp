  ...
  (cond
   ...
   ((case (first x)
      ...
      (cast* (let ((union (second x))
                   (clauses (rest (rest x))))
               (comp-cast union clauses env tenv)))))))

(defun comp-match (x y)
  (cond
    ((or (eql x y) (eq x '*)) t)
    ((and (consp x) (consp y) (comp-match (car x) (car y)))
     (comp-match (cdr x) (cdr y)))
    (t nil)))

(defun comp-matches (types members)
  (let ((matches (remove-duplicates
                  (loop for type in types
                        append (loop for member in members
                                     when (comp-match type member)
                                       collect member))
                  :test #'equal)))
    (when (null (set-difference members matches))
      matches)))

(defun comp-cast (union clauses env tenv)
  (let* ((union-type (satori-type (second union) tenv))
         (union-llvm-type (llvm:pointer-type (llvm-type union-type tenv)))
         (members (rest union-type))
         (types (map 'list #'first clauses))
         (types* (comp-matches types members)))
    (unless types*
      (error 'satori-error :message "non-exhaustive cast"))
    (let* ((tags (map 'list
                      #'(lambda (x)
                          (position x members :from-end t))
                      types*))
           (function (llvm:basic-block-parent (llvm:insertion-block *builder*)))
           (basic-blocks (loop for i from 0 to (length tags)
      ...
      (llvm:build-switch *builder* tag merge-bb (length tags))
      (let* ((bodies (map 'list #'second clauses))
             (nullptr (llvm:const-pointer-null union-llvm-type)))
        (map nil
             #'(lambda (body basic-block)
                 (llvm:position-builder *builder* basic-block)
                 (let ((cbody (first (comp body env tenv))))
                   (llvm:build-ret *builder* cbody)
                   (setf basic-block (llvm:insertion-block *builder*))
                   cbody))
             bodies
             basic-blocks)
        (llvm:position-builder *builder* merge-bb)
        `(,(llvm:build-ret *builder* nullptr) ,env ,tenv)))))
